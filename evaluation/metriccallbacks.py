...
class F1Metric(MetricCallback):
    ...
    def train_val_scores(self):
        ...
        train_predict = (np.asarray(self.model.predict(self.train[0], self.train[1]).argmax(axis=1)))
...
class AUCMetric(MetricCallback):
    ...
    def train_val_scores(self):
        ...
        # print(self.validation[0].shape)
        # print(val_targ)
        val_predict = np.asarray(self.model.predict(self.validation[0], self.validation[1]))
        # print(val_predict)
        train_targ = self.train[1] 
        train_predict = (np.asarray(self.model.predict(self.train[0], self.train[1])))
